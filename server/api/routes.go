				...
				if isValid {
					...
					for i, exp := range msg.Expand {
                        ...
                        if len(token.Parts) > 0 {
                            valid, exp_used_collections := query_lang_anayzer.AnalyzeVariableParts(msg.CollectionName, &token, querylang_types.ANALYZE_VARIABLES_PARTS_ANALYZE_OPTIONS_JOIN | querylang_types.ANALYZE_VARIABLES_PARTS_ANALYZE_OPTIONS_VALIDATE_VARIABLE)
                            if !valid {
					...
					logger.Info("Expand is valid")

                    // analizing SortBy
					analyzed_SortBy := make([]querylang_types.TokenValueVariable, len(msg.SortBy))
					for i, sort_by_field := range msg.SortBy {
						var token querylang_types.TokenValueVariable
						token.Parts = strings.Split(sort_by_field, ".")
                        if len(token.Parts) > 0 {
                            if sort_by_field[0] == '-' || sort_by_field[0] == '+' {
                                token.Parts[0] = token.Parts[0][1:]
                            }

                            valid, sortBy_used_collections := query_lang_anayzer.AnalyzeVariableParts(msg.CollectionName, &token, querylang_types.ANALYZE_VARIABLES_PARTS_ANALYZE_OPTIONS_FILTER | querylang_types.ANALYZE_VARIABLES_PARTS_ANALYZE_OPTIONS_NOT_LIST | querylang_types.ANALYZE_VARIABLES_PARTS_ANALYZE_OPTIONS_FIX | querylang_types.ANALYZE_VARIABLES_PARTS_ANALYZE_OPTIONS_VALIDATE_VARIABLE)
                            if !valid {
                                logger.Info("Expand is not valid")
                                return
                            }
                            used_collections.Union(sortBy_used_collections)
                            analyzed_SortBy[i] = token
                        } else {
                            logger.Info("Expand is not valid")
                            return
                        }
					...
					// running query
					resJson, err := db.RunUserCustomQuery(msg.CollectionName, msg.Fields, tokens, msg.SortBy, analyzed_SortBy, analyzed_expand, msg_limit, msg_offset, used_collections_filters)
                    ...
                    logger.Warn("Filter is not valid")
                    return
                }
			} else {
